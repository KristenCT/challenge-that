...
export default function Article() {
  return (
    ...
    >
      <p>If you operate a loyalty program in Australian hospitality and it has three tiers named something like Gold, Silver, and Bronze &mdash; you&rsquo;re in good company. You&rsquo;re also probably getting significantly less value from your loyalty investment than you should be.</p>
      ...
      <p>Consider two members. The first visits every Friday, brings two friends, and spends consistently. The second visited heavily last year during a renovation at their usual venue, has since returned to their old habits, and now visits once every six weeks. Under most spend-based systems, they may sit in the same tier &mdash; despite having radically different forward-looking value.</p>
      ...
      <p>The fix isn&rsquo;t to abolish tiers. It&rsquo;s to run behavioural segmentation alongside them &mdash; inside them, ideally &mdash; so that your communication and offers are shaped by what a member is likely to do next, not just what they&rsquo;ve done in the past.</p>
      <p>Behavioural segments that consistently drive better outcomes in hospitality include: <strong>rising frequency</strong> (visit rate trending up &mdash; reward the momentum), <strong>lapsed high-value</strong> (was frequent, has slowed &mdash; requires a different reactivation approach than general lapse), <strong>social multiplier</strong> (brings others, even if individual spend is moderate), and <strong>day-part anchored</strong> (highly predictable visit patterns that create targeting opportunities).</p>

      <h2>The tier model that actually works</h2>
      <p>The most effective loyalty structures we&rsquo;ve helped build combine three elements: a visible tier framework that members understand and aspire to, a behavioural segmentation layer that runs beneath it and shapes actual communications, and a set of milestone triggers &mdash; not calendar cadences &mdash; that drive outreach.</p>
      ...
      <p>One thing we hear regularly: &ldquo;our platform doesn&rsquo;t support that kind of segmentation.&rdquo; More often than not, it does &mdash; it just hasn&rsquo;t been configured for it. Our loyalty strategy engagements are platform-agnostic: we design the right structure first, then work with whatever system you have to implement it as fully as possible.</p>
      <p>Where gaps genuinely exist at the platform level, we can assess whether a technology change is warranted &mdash; and help you select and transition to something better suited to a behavioural approach.</p>
    </ArticleLayout>
  )
}
